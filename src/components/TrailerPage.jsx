...
function TrailerPage() {
  ...
  useEffect(() => {
    const fetchMovieDetails = async () => {
      try {
        const [movieRes, trailerRes, creditsRes, similarRes] = await Promise.all([
          GlobalApi.getMovieDetails(movieId),
          GlobalApi.getMovieTrailer(movieId),
      ...
      <div className="relative z-10 px-6 md:px-16 py-12 max-w-6xl mx-auto">
        ...
        >
          ...
          <div className="flex flex-wrap gap-4 text-sm text-gray-300">
            {movie.release_date && (
              <div>
                <span className="text-yellow-400 font-semibold">Release:</span> {movie.release_date}
              </div>
            )}
            {movie.vote_average !== undefined && (
              <div>
                <span className="text-yellow-400 font-semibold">Rating:</span> ⭐ {movie.vote_average.toFixed(1)}
              </div>
            )}
            {movie.runtime && (
              <div>
                <span className="text-yellow-400 font-semibold">Runtime:</span> {movie.runtime} mins
              </div>
            )}
          </div>
          ...
          <div className="mt-12">
            ...
            <div className="flex scrollbar-hide overflow-x-auto gap-6 pb-4">
              {similarMovies.map((smovie) => (
                <div
                  key={smovie.id}
                  onClick={() => navigate(`/trailer/${smovie.id}`)}
                  className="cursor-pointer w-[160px] flex-shrink-0 hover:scale-105 transition-transform duration-300"
                >
                  <img
                    src={IMAGE_BASE_URL + smovie.poster_path}
                    alt={smovie.title}
                    className="rounded-lg shadow-md w-full h-[240px] object-cover"
                  />
                  <p className="text-sm mt-2 text-center text-white font-medium truncate">{smovie.title}</p>
                </div>
              ))}
            </div>
          </div>
